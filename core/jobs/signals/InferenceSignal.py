...
def update_job_inference_model_celery_task_status_and_result(job_celery_task_id, job_celery_task_status, job_celery_task_result=None):
    try:
        job_inference_model_instance = JobInferenceModel.objects.get(
            job_celery_task_id=job_celery_task_id
        )
        if job_inference_model_instance:
            if job_celery_task_status and job_celery_task_result:
                job_inference_model_serializer = JobInferenceModelSerializer(
                    instance=job_inference_model_instance,
                    data={
            ...
            if job_inference_model_serializer.is_valid():
                ...
                print(job_inference_model_serializer.errors)
    except Exception as e:
        print(e)


@task_received.connect(sender=Inference)
def update_on_task_received(sender=None, result=None, **kwargs):
    update_job_inference_model_celery_task_status_and_result(
        job_celery_task_id=str(sender.request.id),
        job_celery_task_status="PENDING"
    )


@task_prerun.connect(sender=Inference)
...
@task_success.connect(sender=Inference)
...
@task_failure.connect(sender=Inference)
...
@task_internal_error.connect(sender=Inference)
...
@task_revoked.connect(sender=Inference)
...
@task_unknown.connect(sender=Inference)
...
@task_rejected.connect(sender=Inference)
