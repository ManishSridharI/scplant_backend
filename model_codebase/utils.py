...
def save_ckpt2(epoch, model, optimizer, scheduler, losses, acc, f1, output_nodes_names, model_name, ckpt_folder):
    """
    save checkpoint
    """
    if not os.path.exists(ckpt_folder):
...
def save_best_ckpt2(epoch, model, optimizer, scheduler, losses, acc, f1, output_nodes_names, model_name, ckpt_folder):
    """
    save checkpoint
    """
    if not os.path.exists(ckpt_folder):
...
def locate_last_epoch(model_path):
    filename = f'{model_path}_best.pth'
    if os.path.exists(filename):
        print(f'found {filename}')
        return filename
    for i in reversed(range(100)):
        filename = f'{model_path}_{i}.pth'
        print(f"check {filename}")
        if os.path.exists(filename):
            return filename
    return ''

def load_ckpt(model, optimizer, scheduler, gpu_id, model_path):
